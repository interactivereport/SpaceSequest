...
This pipeline contains the following major workflows:
 ...
 - **visiumhd**
   ...
   Our pipeline performs analyses related to Visium HD spatial transcriptomics data. It starts the analysis from SpaceRanger bin-level outputs, and runs a series of steps for QC, cell type annotation, and clustering analysis.
  
 - **xenium**
 
   Our pipeline directly takes Xenium data as input, runs through gene/cell level quality control and various downstream analyses.
 
 - **cosmx**
 
   Our pipeline performs CosMx data analysis. This includes quality control, cell type annotation (label transfer from a reference dataset), and clustering analysis.

 - **geomx**
   
   Our pipeline takes GeoMx .dcc files as input, performs sample- and probe-level quality control, and runs a series of steps to call differentially expressed genes. The output will be summarized into [Quickomics](https://academic.oup.com/bioinformatics/article/37/20/3670/6254183){target="_blank"} format for further interactive exploration.
